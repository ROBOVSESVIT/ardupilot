...
local failsafe_start_time_ms -- system time failsafe started (level vehicle)
...
function update()
  ...
  if (not gps_or_ekf_bad) then
    ...
    if home and curr_loc then
      ...
      home_yaw = 180 - math.deg(curr_loc:get_bearing(home))
  ...
  if not arming:is_armed() then 
    ...
    start_yaw = math.deg(ahrs:get_yaw())
  ...
  if (flight_stage == 0) then
    ...
    elseif not rc:has_valid_input() and (home_dist <= enable_dist:get()) then
      ...
      if ((now_ms - failsafe_start_time_ms) <= 5000) then
        vehicle:set_target_angle_and_climbrate(0, 0, target_yaw, 0, false, 0)
        gcs:send_text(5, "DR: leveling vehicle")
      elseif ((now_ms - failsafe_start_time_ms) >= 5000) then
        vehicle:set_target_angle_and_climbrate(0, math.abs(fly_angle:get()), start_yaw, climb_rate, false, 0)
        gcs:send_text(5, "DR: flying back")
      end
    end
